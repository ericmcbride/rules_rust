...
impl<'a> SplicerKind<'a> {
    ...
    fn splice_multi_package(
        ...
        // Optionally install a cargo dot files into the workspace root.
        Self::setup_cargo_dot_files(
            &splicing_manifest.cargo_config,
            workspace_dir.as_std_path(),
            "config",
    ...
    /// A helper for installing Cargo Dot files such as `.cargo/config` or
    /// `.cargo/credentials.toml` into the spliced workspace while also
    ...
    fn setup_cargo_dot_files(
        ...
        while let Some(parent) = current_parent {
            ...
            for config in [
                ...
                if config.exists() {
                    bail!(
                        "A Cargo config dot file was found in a parent directory to the current workspace. This is not allowed because these settings will leak into your Bazel build but will not be reproducible on other machines.\nWorkspace = {}\nCargo dot file = {}",
